...
void printCarData2File(RentalCar arr[]);

/*Sorts cars by ascending price*/
void sortCars(RentalCar arr[]);

/*Asks user how many days desired to rent and shows available cars and price*/
...
int main()
{
  ...
  do
  {
    ...
    switch (menuInput)
    {
      ...
      default:
      {
        cout << "Please enter an option between 1 and 7." << endl;
        break;
      }
    }
  } while(menuInput!=7);
...
void getFile(RentalCar arr[])
{
  ...
  //Scans information into array
  for (int i=0; i<5; i++)
  {
    inFile >> arr[i].year >> arr[i].make >> arr[i].model >> arr[i].price >> arr[i].available;
  }
