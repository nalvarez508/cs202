/**
 * @brief  CS-202 Inheritance Sample
 * @author Christos Papachristos (cpapachristos@unr.edu)
 * @date   February, 2019
 *
 * This file shows a sample implementation of Inheritance syntax, rules, and concepts, for the purposes of CS-202
 */
...
class Planet{
   ...
   *        through Base class pointers, so we also choose to define it and declare it as protected (cannot be called by any EXTERNAL USER or the class but can be used by a Derived class method such
  ...
  protected:
    ...
    Planet& operator=(const Planet& rhs){
      if (this != &rhs){
        ...
        std::cout << "Planet Assignment operator" << std::endl;  // Just a debug statement for the Base class assignment
      }
      return *this;
    }
...
class Earth : public Planet{
  ...
  public:
    ...
    Earth() :
	  ...
	  m_id( 0 )
    {
      std::cout << "Earth Default ctor" << std::endl;  // Just a debug statement for the Derived class Default ctor
    }
...
class SolarSystemPlanet : public Planet{
  ...
  public:
    ...
    SolarSystemPlanet(double g, double au, const char * name ) :
      Planet(g, au)
    {
	  strcpy(m_name, name);
      std::cout << "SolarSystemPlanet Parametrized ctor" << std::endl;  // Just a debug statement for the Derived class Parametrized ctor
    }
  ...
  /* The Derived classes can have their own additional data */
  private:
  
    char m_name[256];  // Just a name for our SolarSystemPlanet
...
int main()
{
  ...
  SolarSystemPlanet solar_system_planet_X(1.52, 3.77, "PlanetX");
  std::cout << solar_system_planet_X << std::endl;

  std::cout << std::endl << "Testing SolarSystemPlanet Copy" << std::endl;
  SolarSystemPlanet solar_system_planet_X_cpy( solar_system_planet_X );
  std::cout << solar_system_planet_X_cpy << std::endl;
   ...
   * If it wasn't (and if the data members were not const so that we could assigne to them ) this would call the Base class Assignment operator=
   * which would only perform assignment of Base class data members (partial assignment of Base class members onlt on a Derived class object)
   * which is most probably an undesired behavior
   */
  //*ss_p = *e_p;
 
  /*
   * Derived class Assignment operator= is not defined (but automatically-synthesized for the Derived class) which will use the Base class Assignment operator=
   * (it is allowed to use it since it is protected in the Base class and the Derived class has access to it).
   */
  SolarSystemPlanet solar_system_planet_X_assign(-1.0, -100.0);  // Arguments passed to Derived class Parametrized ctor will be forwarded to Base class ctor whose
                                                                 // initialization list will not allow negative values to initialize the m_G and m_AU data members

  solar_system_planet_X_assign = solar_system_planet_X;


  std::cout << std::endl << "Tests Done" << std::endl;
  /* Scope end of function main(), expect to see dtor(s) called in order: Derived-dtor -> Base-ctor */
